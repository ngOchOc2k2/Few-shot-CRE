...
logging.basicConfig(filename='./Ngoc/Tacred_5_Test_v3.log',level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
        ...
        for step, batch_data in enumerate(tqdm(data_loader)):
            ...
            if previ_lora == True:
                ...
                if orthogonal_loss != 0:
                    logging.info(f'Yes, have a lora_old')
...
if __name__ == '__main__':
    ...
    for rou in range(config.total_round):
        ...
        for steps, (training_data, valid_data, test_data, current_relations, historic_test_data, seen_relations) in enumerate(sampler):
            logging.info(f'Current relation: {current_relations}')
            ...
            #logging.info(encoder.get_output_size())
            ...
            logging.info(f"Train Simple Finished!")
            ...
            # logging.info(len(train_data_for_initial))
            logging.info(f"Length expanded train init: {len(expanded_train_data_for_initial)}")
            ...
            logging.info(f"Train First Finished!")
            ...
            logging.info(f'Length seen relation: {len(seen_relations)}')
            logging.info(f'Train data memory: {len(train_data_for_memory)}')
            ...
            logging.info(f"Train Memory Finished!")
            ...
            logging.info(f"Length test current task: {len(test_data_1)}")
            logging.info(f"Length test all task: {len(test_data_2)}")
            ...
            logging.info(f'Restart Num {rou + 1}')
            logging.info(f'Task--{steps + 1}:')
            logging.info(f'Current test acc: {cur_acc}')
            logging.info(f'History test acc: {total_acc}')
            logging.info(f'Accuracy_test_current: {test_cur}')
            logging.info(f'Accuracy_test_total: {test_total}')
            ...
            logging.info(f"Accuracy_per_task_current_test: {accuracy}")
        ...
        logging.info(f"Result_whole_test: {result_whole_test}")
        logging.info(f"Avg_result_cur_test: {avg_result_cur_test}")
        logging.info(f"Avg_result_all_test: {avg_result_all_test}")
        logging.info(f"Std_result_all_test: {std_result_all_test}")
        ...
        logging.info(f"Accuracy_result_all_test: {accuracy}")
        ...
        logging.info(f"Bwt_whole: {bwt_whole}")
        logging.info(f"Fwt_whole: {fwt_whole}")
        logging.info(f"Avg_bwt_whole: {avg_bwt}")
        logging.info(f"Avg_fwt_whole: {avg_fwt}")
